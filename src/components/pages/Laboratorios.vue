...
<script>
  ...
  export default {
    ...
    head: {
      title() {
        return {
          inner: `${config.SITE_TITLE} - Laboratorios`
        };
      },
      meta: [
        {
    ...
    watch: {
      filter(data) {
        if (data.key != this.optionsInput[0].key && data.key != '') {
          this.listaFiltrada = window._.filter(this.listaItens, (obj) => (obj.key == data.key));
        } else {
    ...
    methods: {
      async getLabs() {
        ...
        try {
          ...
          for (let i = 0; i < this.listaFiltrada.length; i++) {
            this.listaFiltrada[i].subitens = window._.orderBy(this.listaFiltrada[i].subitens, ['nome'], ['asc']);
          }
        } catch (err) {
          console.log(err);
        }
      ...
      async getLab(id) {
        try {
          ...
          Object.keys(response.data.laboratories).forEach((laboratorie) => {
            Object.keys(response.data.laboratories[laboratorie].laboratories).forEach((lab) => {
              dictionary[0][1] = response.data.laboratories[laboratorie].laboratories[lab].name;
              dictionary[1][1] = `${response.data.laboratories[laboratorie].initials} - ${response.data.laboratories[laboratorie].institution}`;
              dictionary[2][1] = response.data.laboratories[laboratorie].laboratories[lab].email;
              dictionary[3][1] = response.data.laboratories[laboratorie].laboratories[lab].homepage;
              dictionary[4][1] = response.data.laboratories[laboratorie].laboratories[lab].phone;
              dictionary[5][1] = response.data.laboratories[laboratorie].laboratories[lab].description;
              dictionary[6][1] = response.data.laboratories[laboratorie].laboratories[lab].responsables;
              this.labId = response.data.laboratories[laboratorie].laboratories[lab].id;
              this.modal = response.data.laboratories[laboratorie].laboratories[lab].id;
            });
          });
      ...
      async handleInput() {
        ...
        try {
          ...
          console.log(err);
          this.search = 'Ocorreu um erro durante a busca, por favor tente mais tarde.';
        } finally {
