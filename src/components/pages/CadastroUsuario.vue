...
<script>
  ...
  import Cidades from '../../scripts/services/Cidades';
  import Estados from '../../scripts/services/Estados';
  ...
  export default {
    ...
    methods: {
      ...
      validaFormulario() {
        ...
        const erNascimento = /^(((0[1-9]|[12]\d|3[01])\/(0[13578]|1[02])\/((19|[2-9]\d)\d{2}))|((0[1-9]|[12]\d|30)\/(0[13456789]|1[012])\/((19|[2-9]\d)\d{2}))|((0[1-9]|1\d|2[0-8])\/02\/((19|[2-9]\d)\d{2}))|(29\/02\/((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))))$/g;
        ...
        if (this.dadosFormulario.curriculum === '') {
          ...
          const curriculumTratado = this.dadosFormulario.curriculum.substring(0, 22);
          if (curriculumTratado !== 'http://lattes.cnpq.br/') {
            this.mensagensErro.erroCurriculum = 'Link inválido, deve ser no formato "http://lattes.cnpq.br/..."';
            status = false;
          } else {
            this.mensagensErro.erroCurriculum = '';
          }
        ...
        if (this.dadosFormulario.cpf === '') {
          ...
          status = false;
        } else if (!erCpf.exec(this.dadosFormulario.cpf) || this.dadosFormulario.cpf === '111.111.111-11' ||
          this.dadosFormulario.cpf === '222.222.222-22' || this.dadosFormulario.cpf === '333.333.333-33' ||
          this.dadosFormulario.cpf === '444.444.444-44' || this.dadosFormulario.cpf === '555.555.555-55' ||
          this.dadosFormulario.cpf === '666.666.666-66' || this.dadosFormulario.cpf === '777.777.777-77' ||
          this.dadosFormulario.cpf === '888.888.888-88' || this.dadosFormulario.cpf === '999.999.999-99' ||
        ...
        if (this.dadosFormulario.segundaSenha === '') {
          ...
          status = false;
        } else if (this.dadosFormulario.segundaSenha !== this.dadosFormulario.senha) {
        ...
        if (!geraExpressao.validarExpressao(this.expressao, this.resultadoExpressao)) {
          this.mensagensErro.erroExpressao = 'O resultado da expressão não está correto';
          status = false;
        } else {
          this.mensagensErro.erroExpressao = '';
        }
      ...
      cadastroUsuario() {
        if (this.validaFormulario()) {
          Cadastro.postCadastro(this.dadosFormulario)
            .then(response => {
      ...
      getEstados() {
        Estados.getEstados()
          .then(response => {
            for (const estado in response.data.registers) {
              this.estados.push(response.data.registers[estado]);
            }
          });
      },
      getCidades(event) {
        this.cidades = [];
        Cidades.getCidades(event)
          .then(response => {
            for (const cidade in response.data.citys) {
              this.cidades.push(response.data.citys[cidade]);
            }
          });
      }
    }
