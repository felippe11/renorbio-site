    ...
    watch: {
      filter(data) {
        ...
        if (data.name != this.optionsInput.todos && data.name != '') {
          this.listaFiltrada = this.ordenarPorPF(window._.filter(this.listaItens, (iten) => (iten.tipo == data.name)));
        } else {
    ...
    methods: {
      ordenarPorPF(itens) {
        ...
        Object.keys(obj).forEach((key) => {
            ...
            listaFiltrada[listaFiltrada.length - 1].subitens = thisGlobal.ordenaAsc(listaFiltrada[listaFiltrada.length - 1].subitens);
        });
      ...
      getTeachers(id) {
        ...
        Teachers.getTeachers(id)
          .then(response => {
            for (const i in response.data.registers) {
              const teacherTratado = {
                point: response.data.registers[i].nucleator,
                teachers: response.data.registers[i].teachers,
              };
              teacherTratado.teachers = window._.orderBy(teacherTratado.teachers, ['name'], ['asc']);
      ...
      getAll() {
        ...
        Teachers.getTeachers('all')
          .then(response => {
            for (const i in response.data.registers) {
              const teacherTratado = {
                name: response.data.registers[i].nucleator,
                subitens: response.data.registers[i].teachers,
              };
              teacherTratado.subitens = window._.orderBy(teacherTratado.subitens, ['name'], ['asc']);
      ...
      async handleInput(key) {
        ...
        if (this.input.length >= 1 && this.input.length <= 2) {
          ...
          try {
            const {data: {registers}} = await Teachers.searchTeacher(key, this.input);
