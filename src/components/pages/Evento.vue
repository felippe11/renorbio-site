...
<script>
  ...
  export default {
    ...
    mounted() {
      this.loadEvento(this.$route.params.id);
      this.getAsyncData();
    },
    methods: {
      loadEvento(id) {
        ...
        .then(evento => {
          ...
          this.imagem = (evento.featured_media) ? evento._embedded['wp:featuredmedia'][0].source_url : false;
          this.imagemCaption = (evento.featured_media) ? evento._embedded['wp:featuredmedia'][0].caption.rendered : false;

          this.eventosDados.inicio = DataUtil.dataInversa(evento.acf.data_inicio);
          this.eventosDados.fim = (evento.acf.data_fim) ? DataUtil.dataInversa(evento.acf.data_fim) : false;
          this.eventosDados.email = evento.acf.email_de_contato;
          this.eventosDados.horario = (evento.acf.horario) ? evento.acf.horario : false;
          ...
          if (this.eventosDados.site) {
            if (!this.eventosDados.site.includes(http) && !this.eventosDados.site.includes(https)) {
              this.eventosDados.site = http + this.eventosDados.site;
            }
          }
          this.eventosDados.telefone = (evento.acf.telefone_de_contato) ? evento.acf.telefone_de_contato : false;
