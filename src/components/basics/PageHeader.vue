      ...
      .container(:class="{'container-mobileOrTablet' : isMobileOrTablet}")
        .barraSuperior
          ...
          .opcoes(v-show="!menuMobile")
            ...
            router-link(:to="{ name: 'Mapa' }" :tabindex="tab+5") #[i.fa.fa-sitemap.link(aria-hiden="true" alt="Ver mapa do site" title="Ver mapa do site")]
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      logoCentral() {
        return (!this.internal || this.isMobileOrTablet) && (!this.menuMobileIsOpened || !this.isMobileOrTablet);
      },
      langsMenu: () => [{sigla: 'PT', descricao: 'Versão Português'}, {sigla: 'EN', descricao: 'Versão Inglês'}, {sigla: 'ES', descricao: 'Versão Espanhol'}],
      menuMobile() {
...
<style lang="scss" scoped>
  ...
  nav.header {
    background: url('../../../public/renorbio.imagem-topo.png') no-repeat center top;
