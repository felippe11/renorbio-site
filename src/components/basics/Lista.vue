...
<script>
  ...
  import Teachers from '../../scripts/services/Teachers';
    ...
    methods: {
      ...
      async getDocente(id) {
        try {
          ...
          const response = await Teachers.getTeacher(id);
          Object.keys(response.data.registers).forEach((register) => {
            Object.keys(response.data.registers[register].teachers).forEach((teacher) => {
              dictionary[0][1] = response.data.registers[register].teachers[teacher].name;
              dictionary[1][1] = response.data.registers[register].nucleator;
              dictionary[2][1] = response.data.registers[register].teachers[teacher].email;
              dictionary[3][1] = response.data.registers[register].teachers[teacher].areas;
              dictionary[4][1] = this.replaceInvalidCharacter(response.data.registers[register].teachers[teacher].occupation_area);
              dictionary[5][1] = response.data.registers[register].teachers[teacher].services;
              dictionary[6][1] = response.data.registers[register].teachers[teacher].lattes;
              dictionary[7][1] = response.data.registers[register].teachers[teacher].laboratories;
              this.docenteName = response.data.registers[register].teachers[teacher].name;
              this.modal = response.data.registers[register].teachers[teacher].name;
            });
          });
      ...
      show(estadoNome, estado, docente) {
        ...
        const dictionary = [
          ...
          ['Área de atuação', this.replaceInvalidCharacter(estado.occupation_area), 'string'],
